...
public:
	...
	BOOL SetCurrentDimStyle( char*  Name );
	...
	BOOL Text( char*  Text, double x1, double y1, double x2, double y2, double Height, DWORD Justification, double Rotation=0, double WidthFactor=0, char*  StyleName=NULL );
	BOOL Text( char*  Text, double x, double y, double Height, double Rotation=0, double WidthFactor=0, char*  StyleName=NULL );
	BOOL DimLinear( double x1, double y1, double x2, double y2, double x3, double y3, double angle, char*  text=NULL );
	BOOL Arc( double cx, double cy, double r, double StartAngle, double EndAngle, double Thickness=0.0 );
	BOOL InsertBlock( char*  BlockName, double x, double y, double xScale=1, double yScale=1, double Rotation=0 );
	
	// BLOCKS section
	BOOL BlockBegin( char*  BlockName, char*  LayerName, double bx, double by, char flags=0);
	BOOL BlockEnd();

	BOOL WriteParameter( int GroupCode, char*  Value );
...
private:
	...
	BOOL (*dxfText)( HDXF hDxf, char*  Text, double x1, double y1, double x2, double y2, double Height, double Rotation, DWORD Justification, double WidthFactor, char*  StyleName );
	BOOL (*dxfDimLinear)( HDXF hDxf, double x1, double y1, double x2, double y2, double x3, double y3, double angle, char*  text );
	BOOL (*dxfAddDimStyle)( HDXF hDxf, PDXFDIMSTYLE pDimStyle );
	BOOL (*dxfSetCurrentDimStyle)( HDXF hDxf, char*  Name );
	BOOL (*dxfArc)( HDXF hDxf, double cx, double cy, double r, double StartAngle, double EndAngle, double Thickness );
	BOOL (*dxfInsertBlock)( HDXF hDxf, char*  BlockName, double x, double y, double xScale, double yScale, double Rotation );
	BOOL (*dxfBlockBegin)( HDXF hDxf, char*  BlockName, char*  LayerName, double bx, double by, char flags );
...
public:
	...
	// Configuration
	BOOL SetLayer( char* Name );
	BOOL SetLineType( char*  Name );
	BOOL SetTextStyle( char* Name );
	BOOL SetDimStyle( char* Name );
...
//	BOOL ChangeTableType( DWORD dwTableType, OBJHANDLE TableTypeObjhandle, LPVOID pTableType );
	...
	// BLOCKS section
	OBJHANDLE AddBlock( PBLOCKHEADER pBlockHeader, char*  strFileName=NULL );
//	BOOL ChangeBlock( OBJHANDLE BlockObjhandle, PBLOCKHEADER pBlockHeader );
	...
	BOOL SetActiveBlock( char* strBlockName );
	OBJHANDLE GetActiveBlock( );

	// ENTITIES section
	OBJHANDLE AddEntity( OBJHANDLE BlockObjhandle, PENTITYHEADER pEntityHeader, LPVOID pEntityData );
	BOOL ChangeEntity( PENTITYHEADER pEntityHeader, LPVOID pEntityData, char* strBlockName=NULL );
	BOOL DeleteEntity( char* strBlockName, OBJHANDLE EntityObjhandle );
	OBJHANDLE FindEntity( DWORD dwFindType, PENTITYHEADER pEntityHeader, LPVOID pEntityData, char*  strBlockName=NULL );
...
//	OBJHANDLE Ellipse( double cx, double cy, double ex, double ey, double Ratio, double StartParam=0.0, double EndParam=2*PI );
	OBJHANDLE DimLinear( double x1, double y1, double x2, double y2, double x3, double y3, double angle, char*  text=NULL );
	OBJHANDLE InsertBlock( OBJHANDLE BlockObjhandle, double x, double y, double xScale=1, double yScale=1, double Rotation=0 );
	OBJHANDLE InsertBlock( char*  BlockName, double x, double y, double xScale=1, double yScale=1, double Rotation=0 );
	...
	OBJHANDLE Text( char* Text, double x1, double y1, double x2, double y2, double Height, short Justification, double Rotation=0, double WidthFactor=0 );
	OBJHANDLE Text( char* Text, double x, double y, double Height, double Rotation=0, double WidthFactor=0 );
	OBJHANDLE PolyLine( PENTVERTEX pVertex, int nVertex, unsigned short Flag=0 );

	// Datafile data-exchange
	BOOL SaveDXFFile( char*  FileName, HWND hWndProgress=NULL );
	BOOL LoadDXFFile( char*  FileName, HWND hWndProgress=NULL );
...
private:
	...
	BOOL (*drwDeleteEntity)( HDRAWING hDrawing, char* strBlockName, OBJHANDLE EntityHandle );
	BOOL (*drwChangeEntity)( HDRAWING hDrawing, char* strBlockName, PENTITYHEADER pEntityHeader, LPVOID pEntityData );
	OBJHANDLE (*drwFindEntity)( HDRAWING hDrawing, char*  strBlockName, PENTITYHEADER pEntityHeader, LPVOID pEntityData, DWORD dwFindType );

	BOOL (*drwSaveDataToFile)( HDRAWING hDrawing, DWORD Reserved, char*  strFileName, HWND hWndProgress );
	BOOL (*drwLoadDataFromFile)( HDRAWING hDrawing, OBJHANDLE BlockObjhandle, DWORD Reserved, char*  strFileName, HWND hWndProgress );
