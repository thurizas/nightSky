...
BOOL drwSaveDataToFile( HDRAWING hDrawing, DWORD Reserved, LPCTSTR strFileName, HWND hWndProgress )
{
	...
	DXFLTYPE	ltype;
	for(unsigned int i=0; i<pDrawing->Tables.NumLTypes; i++)
	{
	...
	for(unsigned int i=0; i<pDrawing->Tables.NumLayers; i++)
	{
		LTYPE	LType;
		char*	pLTypeName;
	...
	DXFSTYLE tstyle;
	for(unsigned int i=0; i<pDrawing->Tables.NumStyles; i++)
	{
	...
	for(unsigned int i=0; i<pDrawing->Tables.NumDimStyles; i++)
	{
		STYLE	Style;
		char*	pStyleName;
